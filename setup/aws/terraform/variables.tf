...
variable "name_prefix" {
  description = "Prefix used for resource names and the EC2 Name tag."
  type        = string
  default     = "vault-repro"
}
...
variable "admin_ssh_cidr" {
  description = "CIDR allowed to SSH to the instance (for example your public IP as <ip>/32)."
  type        = string
}
...
variable "vault_version" {
  description = "Vault Enterprise version to install, including the +ent suffix (for example 1.20.2+ent)."
  type        = string
  default     = "1.20.2+ent"
}
