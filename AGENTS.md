...
- `AGENTS.md` (this file) — project rules. Read by opencode, Cursor, modern IBM agents, and any tool that follows the AGENTS.md convention.
...
- `.ai/skills/` — reusable agent skills (Anthropic Agent Skills format with `name` + `description` frontmatter):
    - `customer-reply/SKILL.md` — drafting Vault support replies.
...
When updating authoring rules, edit the file under `.ai/` (or this `AGENTS.md` for project-wide rules). The thin adapter files exist under `.github/`, `.cursor/`, and `.opencode/` should remain pointers and not accumulate drift.
